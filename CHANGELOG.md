...
- **Studio (`ngmt-studio` repo, 2026-04):** Generator **wire-aligned** central preview (pre-encode BGRX → texture, preview max dimension **1280 px**), expanded **send HUD** (subscribers, `last_object_id`, pattern / VMX / mDNS summary), **validated editable mDNS instance** + **unregister** on stream end; Monitor **templates** materialize **idle** slots with **hatched** placeholders + index. **`ngmt-studio-common`:** `dns_sd` validation helpers, **`unregister_source`**. See [ngmt-studio/CHANGELOG.md](ngmt-studio/CHANGELOG.md).
