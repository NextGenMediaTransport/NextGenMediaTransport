...
- **`ngmt-capture`:** standalone org repo ([`NextGenMediaTransport/ngmt-capture`](https://github.com/NextGenMediaTransport/ngmt-capture)) — workspace with `crates/ngmt-capture` + `crates/ngmt-vmx-sys`; macOS ScreenCaptureKit → VMX → QUIC + mDNS `role=capture`; Linux/Windows stub; meta CI clones it and runs `cargo check` on Ubuntu (with `ngmt-codec` + GUI deps).
