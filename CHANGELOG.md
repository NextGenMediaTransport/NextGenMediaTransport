...
- **`ngmt-obs-plugin` + `ngmt-transport`:** OBS input **foundation** for [ngmt-obs-plugin#4](https://github.com/NextGenMediaTransport/ngmt-obs-plugin/issues/4) — **`obs-ngmt`** module with **NGMT Source** (`ngmt_input`), manual/discovery property UX (defaults **127.0.0.1:4433**), stub discovery list, **`blog()`** bridge via new transport FFI **`ngmt_transport_set_log_fn`** / **`ngmt_transport_try_init_tracing_forwarder`**, CMake link to sibling **cdylib** + post-build copy for load path. See [ngmt-obs-plugin/README.md](ngmt-obs-plugin/README.md), [ngmt-obs-plugin/CHANGELOG.md](ngmt-obs-plugin/CHANGELOG.md), [ngmt-transport/CHANGELOG.md](ngmt-transport/CHANGELOG.md), and [Phase 5](docs/project-plan/05-Phase-5-Integrations-and-Ecosystem.md).
