...
| **3** — Discovery & WAN | mDNS, QUIC, harness, WAN honesty | **In progress** (per [plan](./03-Phase-3-Core-Features-Discovery-and-WAN.md)) | **Rust:** `ngmt-transport` (QUIC, BBR, FFI, ALPN `ngmt`), `ngmt-studio` mDNS `_ngmt._udp`. **TLS:** operator PEM / pinning knobs in [`ngmt-transport` README](../../ngmt-transport/README.md#tls-modes-lab-vs-operator-pem) (lab default unchanged). **C++:** `ngmt-core` discovery is **facade + manual** until v1.0 parity ([discovery status](../../ngmt-core/docs/discovery-status.md)); escape hatch doc: [ngmt-core-discovery-escape-hatch.md](./ngmt-core-discovery-escape-hatch.md). **Impairment:** [impairment-results.md](../testing/impairment-results.md) has **2% / 5% / 10%** profile rows (metrics pending lab). **Not in v1 transport yet:** STUN/TURN/ICE. |
...
| **5** — Integrations | OBS, virtual camera/audio, SDKs | **In progress** (OBS input path) | **`ngmt-obs-plugin` input:** **VMX v1** + LAN discovery on **Windows / macOS / Linux** in `main` ([README](https://github.com/NextGenMediaTransport/ngmt-obs-plugin/blob/main/README.md), [TRACKING](https://github.com/NextGenMediaTransport/ngmt-obs-plugin/blob/main/TRACKING.md)). **Public beta** bar = known-good matrix + TLS / cross-OS / retail-OBS polish in the tracker; see [Phase 5](./05-Phase-5-Integrations-and-Ecosystem.md). Virtual camera/audio still scoped. **Post-v1.0 / Phase 5b:** Python and other SDK wrappers — not gating v1.0. |
...
| ------ | ----------------- |
| **Real Media Path** | End-to-end **VMX / ngmt-codec** encoding → **QUIC** (`ngmt-transport`) → **decoding** — **no** primary-path **stub transport** (e.g. ASCII `frame=N` bodies). The **Studio** Generator ↔ Monitor path meets this for **lab** tooling; **`ngmt-obs-plugin` input** on **`main`** uses **VMX v1** on the same wire. **v1.0** still requires **pillar closure** (TLS, impairment audit, cross-OS operator validation, virtual devices where in scope) per the other pillars and [gap list](#missing-for-version-10-gap-list). |
...
**Lab ordering (2026-04):** **`ngmt-capture`** macOS path is **in active iteration** (**0.2.0**: display picker, multi-session, Studio-aligned UI — see [ngmt-capture-spec](./ngmt-capture-spec.md) and [studio-next-steps](./studio-next-steps.md)). **v1.0** work remains the **vertical slice** (OBS P0, TLS, impairment audit, C++ discovery) in parallel; capture does **not** replace the Four Pillars. **Monitor multiview mirror to a physical display** is **deferred** until the lab can **validate multi-monitor output** on real hardware.
...
| - | -------------- | --- | ----- | ----- |
| 1 | **Real Media Path** | **OBS input** already runs **media payload v1 + VMX** over QUIC in **`ngmt-obs-plugin` `main`**. Close the pillar with **operator-grade** delivery: **TLS** (row **#3**), **impairment metrics** (row **#4**), cross-OS / retail-OBS confidence (overlaps **#2**), **virtual camera** if in scope (**2b**) — not further “stub video” work on the primary OBS path. | 5 | [ngmt-obs-plugin TRACKING](https://github.com/NextGenMediaTransport/ngmt-obs-plugin/blob/main/TRACKING.md); Studio Generator/Monitor remain lab tools. |
...
| 3 | **Security Baseline** | **TLS / PKI policy** for production (pinning / user PEM / upgrade path); replace **lab-only** rcgen defaults for shipped production-oriented binaries | 3 / release | **Progress:** `NGMT_TLS_*` PEM env vars in `ngmt-transport` ([README](../../ngmt-transport/README.md)); **remaining:** Studio/OBS operator UX + “secure by default” product decision for **1.0.0**. Not “build a CA”; see [v1.0 realities](#v10-realities). |
| 4 | **Audit of Honesty** | [impairment-results.md](../testing/impairment-results.md) populated for **2%, 5%, and 10%** loss (document tool, revision, **OS + hardware**, metrics). Prefer **at least two** platform classes when practical (e.g. **x86 Linux + Apple ARM**) | 3 | **Progress:** log lists **2% / 5% / 10%** netem profiles + transport SHA with **pending metrics** until lab runs land. NIC jitter differs; compare [Fedora/M2](#v10-realities). |
| 5 | **Discovery** | **C++ mDNS parity** with Studio / `_ngmt._udp`, **or** shipped migration doc + **Rust→FFI** discovery feed (see [Phase 3](./03-Phase-3-Core-Features-Discovery-and-WAN.md)) | 3 | [discovery-status.md](../../ngmt-core/docs/discovery-status.md). **Doc:** [ngmt-core-discovery-escape-hatch.md](./ngmt-core-discovery-escape-hatch.md). |
...
| — | *(non-pillar)* | **Phase 2** meta-build script, **CONTRIBUTING** rigor | 2 | **Progress:** root [CONTRIBUTING.md](../../CONTRIBUTING.md). Adoption friction; not a Four Pillar blocker. |
...
**GitHub milestones:** Each org repository uses open milestones **`v1.0`** (release gate: close before tagging **1.0.0**) and **`v1.0-adoption`** (tooling and polish not required for the tag per [gap list](#missing-for-version-10-gap-list)). Browse **Issues → Milestones** on [NextGenMediaTransport](https://github.com/orgs/NextGenMediaTransport/repositories).

**GitHub tracking (2026-04):** Program board [NextGenMediaTransport/NextGenMediaTransport#3](https://github.com/NextGenMediaTransport/NextGenMediaTransport/issues/3). Gap rows above map to: **#1** real media path → [ngmt-codec#1](https://github.com/NextGenMediaTransport/ngmt-codec/issues/1) + OBS input epic [ngmt-obs-plugin#1](https://github.com/NextGenMediaTransport/ngmt-obs-plugin/issues/1) (children [#4](https://github.com/NextGenMediaTransport/ngmt-obs-plugin/issues/4)–[#9](https://github.com/NextGenMediaTransport/ngmt-obs-plugin/issues/9)); **#2** OBS P0 → [ngmt-obs-plugin#1](https://github.com/NextGenMediaTransport/ngmt-obs-plugin/issues/1); **2b** virtual devices → [#16](https://github.com/NextGenMediaTransport/ngmt-obs-plugin/issues/16) [#17](https://github.com/NextGenMediaTransport/ngmt-obs-plugin/issues/17); **#3** TLS → [ngmt-transport#1](https://github.com/NextGenMediaTransport/ngmt-transport/issues/1); **#4** impairment log → [meta#1](https://github.com/NextGenMediaTransport/NextGenMediaTransport/issues/1); **#5** discovery → [ngmt-core#1](https://github.com/NextGenMediaTransport/ngmt-core/issues/1); STUN/TURN row → [ngmt-transport#2](https://github.com/NextGenMediaTransport/ngmt-transport/issues/2); Phase 2 meta tooling → [meta#2](https://github.com/NextGenMediaTransport/NextGenMediaTransport/issues/2); telemetry/tests → [meta#5](https://github.com/NextGenMediaTransport/NextGenMediaTransport/issues/5); Phase 6 hardware → [meta#4](https://github.com/NextGenMediaTransport/NextGenMediaTransport/issues/4). **Shippable extras** (semver, wire format versioning, cross-repo CHANGELOGs, integration-guide updates) → [meta#6](https://github.com/NextGenMediaTransport/NextGenMediaTransport/issues/6).
