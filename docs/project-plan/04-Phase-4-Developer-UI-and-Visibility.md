...
- **Repository:** [`ngmt-studio`](../../ngmt-studio/README.md) is a **dedicated Git repository** (sibling to `ngmt-core`, `ngmt-codec`, `ngmt-transport` in the meta-workspace). It contains a Cargo workspace: **`ngmt-generator`**, **`ngmt-monitor`**, **`ngmt-common`** (shared discovery, media, tracing, theme).
...
- **Discovery:** **`_ngmt._udp`** via [`mdns-sd`](https://crates.io/crates/mdns-sd) in `ngmt-common` (browse + register). See [DNS-SD](../protocol/ngmt-wire-format.md#dns-sd) in the wire format doc for instance name and TXT semantics.
- **Primary media path:** QUIC **datagrams** carry **VMX** payloads per [media payload v1](../protocol/ngmt-wire-format.md#media-payload-v1-vmx-video--studio-primary-path) (NGMT object header + fragmented bitstream, `origination_timestamp_us`, width/height on fragment 0). **Generator** encodes test-pattern **BGRX** with **`VMX_EncodeBGRX`** / **`VMX_SaveTo`** and sends; **Monitor** reassembles, **`VMX_LoadFrom`** / **`VMX_DecodeBGRX`**, and uploads textures for preview. **Generator central preview** mirrors the **pre-encode BGRX** frame (same output resolution and pattern phase as the VMX path while streaming; idle uses the same pattern fill at the selected output size). **Mock preview** in Monitor draws **local** SMPTE bars for UI-only layout testing when no feed is present. **Multiview templates** can materialize **idle** canvas slots (`SlotTarget::None`) with hatched placeholders until a source is bound. Solo/maximize is **manual** (⛶), not automatic on connect.
